...
async function createGroup(page, options) {
  if (options.group) {
    ...
    if (await page.$('label[aria-label="Page name (required)"]')) {
      ...
      await page.type(
        'label[aria-label="Page name (required)"] input',
        options.group.name,
        { delay: 95 }
      );
      await page.type(
        'label[aria-label="Category (required)"] input',
...
async function addBM(password, proxy_login, proxy_password, browser, options) {
  if (options.bm) {
    ...
    await bmPage.authenticate({
      username: proxy_login,
      password: proxy_password,
    });
    await bmPage.goto("https://www.facebook.com");
    await bmPage.waitForTimeout(1000);
    const element = await bmPage.waitForSelector(
      'div[data-pagelet="LeftRail"] span span'
    );
    ...
    if ((await bmPage.$('input[placeholder="First and Last Name"]')) !== null) {
      await bmPage.type('input[placeholder="First and Last Name"]', text, {
        delay: 120,
      });
  ...
  if (cookie) {
    ...
    if ((await page.$("#pasteButton")) !== null) {
      ...
      if ((await page.$("#pasteCookie textarea")) !== null) {
        await page.$eval(
          "#pasteCookie textarea",
          (el, value) => (el.value = JSON.stringify(value)),
          cookies
        );
        await page.waitForTimeout(1000);
    ...
    try {
      if ((await frame.$("#contentCol")) !== null) {
        if ((await frame.$("#contentCol #quality_setting")) !== null) {
          ...
          if (
            (await frame.$(
              'div:not(.hidden_elem).uiContextualLayerPositioner.uiLayer ul[role="menu"]'
            )) !== null
          ) {
            await frame.click(
              'div:not(.hidden_elem).uiContextualLayerPositioner.uiLayer ul[role="menu"] li:nth-child(2)'
            );
            await frame.waitForTimeout(1000);
          }
        }
        if ((await frame.$("#contentCol #autoplay_setting")) !== null) {
          ...
          if (
            ...
            await frame.click(
              'div:not(.hidden_elem).uiContextualLayerPositioner.uiLayer ul[role="menu"] li:nth-child(3)'
            );
            await frame.waitForTimeout(1000);
          }
        }
...
async function createBM(
  ...
  if (options.createBM) {
    ...
    await pageCreateBM.authenticate({
      username: proxy_login,
      password: proxy_password,
    });
    await pageCreateBM.goto("https://business.facebook.com/overview", {
      waitUntil: "load",
      timeout: 60000,
    });
    ...
    let text = await pageCreateBM.evaluate(
    ...
    await pageEmail.authenticate({
      username: proxy_login,
      password: proxy_password,
    });
    const address = email.split(/@/);
    if (address[1] === "hotmail.com") {
      await pageEmail.goto("https://outlook.live.com/", {
        waitUntil: "load",
        timeout: 60000,
      });
      ...
      await pageEmail.waitForTimeout(5000);
      await pageEmail.type('input[name="passwd"]', email_password, {
        delay: 130,
      });
      await pageEmail.click('input[type="submit"]');
      await pageEmail.waitForTimeout(5000);
      if ((await pageEmail.$('div[id="StartHeader"]')) !== null) {
        ...
        if (
          (await pageEmail.$('select[id="DisplayPhoneCountryISO"]')) !== null
        ) {
      ...
      try {
        ...
        const response = await axios.get(
          `https://sms-activate.ru/stubs/handler_api.php?api_key=${api}&action=getNumber&service=mm&country=10`
        );
        console.log(response.data);
        const data = response.data.split(/:/);
        if (data[1] && data[2]) {
          if ((await pageEmail.$('input[id="DisplayPhoneNumber"]')) !== null) {
            await pageEmail.type(
              'input[id="DisplayPhoneNumber"]',
              data[2].slice(2),
              { delay: 115 }
            );
            await pageEmail.click('input[type="submit"]');
          ...
          async function getCode() {
            for (let i = 0; i < 2; i++) {
              ...
              const response = await axios.get(
                `https://sms-activate.ru/stubs/handler_api.php?api_key=${api}&action=getStatus&id=${data[1]}`
              );
              if (response.data !== "STATUS_WAIT_CODE") {
                const code = response.data.split(/:/)[1];

                if ((await pageEmail.$('input[id="iOttText"]')) !== null) {
                  await pageEmail.type('input[id="iOttText"]', code, {
                    delay: 115,
                  });
                  await pageEmail.waitForTimeout(1000);
                  await pageEmail.click('input[type="submit"]');
                  ...
                  await pageEmail.type(
                    'input[aria-label="Enter the access code"]',
                    code,
                    { delay: 115 }
                  );
                  await pageEmail.click('input[type="submit"]');
...
async function toEmail(proxy_login, proxy_password, email, email_password, browser) {
  ...
  await pageGmail.authenticate({
    username: proxy_login,
    password: proxy_password,
  });
  await pageGmail.goto("https://gmail.com/", {
    waitUntil: "load",
    timeout: 60000,
  });
            ...
            if (response.data !== "STATUS_WAIT_CODE") {
              ...
              if ((await pageEmail.$('input[id="iOttText"]')) !== null) {
                ...
                if ((await pageEmail.$('input[name="proof"]')) !== null) {
                  ...
                  await axios.get(
                    `https://sms-activate.ru/stubs/handler_api.php?api_key=${api}&action=setStatus&status=3&id=${data[1]}`
                  );
                ...
                if (
                  (await pageEmail.$('select[id="iProofOptions"]')) !== null
                ) {
                  await pageEmail.select('select[id="iProofOptions"]', "Phone");
                  await pageEmail.waitForTimeout(2000);
                }
                if (
                  (await pageEmail.$('select[id="DisplayPhoneCountryISO"]')) !==
                  null
                ) {
                  await pageEmail.select(
                ...
                if (
                  (await pageEmail.$('input[id="DisplayPhoneNumber"]')) !== null
                ) {
                  await pageEmail.type(
                    'input[id="DisplayPhoneNumber"]',
                    data[2].slice(2),
                  ...
                  if ((await pageEmail.$('input[id="iOttText"]')) !== null) {
                    ...
                    if ((await pageEmail.$('input[name="proof"]')) !== null) {
                      ...
                      if (response.data) {
                        const code = response.data.split(/:/)[1];
                        if (
                          (await pageEmail.$('input[id="iOttText"]')) !== null
                        ) {
                          await pageEmail.type('input[id="iOttText"]', code, {
                            delay: 115,
                          });
  ...
  try {
    ...
    try {
      response = await axios.get(
        `http://localhost.multiloginapp.com:${mlaPort}/api/v1/profile/start?automation=true&puppeteer=true&profileId=${uuid}`
      );
    } catch (e) {
      return bulk
        ? { [login]: { status: "Multilogin error" } }
        : res.send({ status: "Multilogin error" });
    }
    await delay(1000);
    if (response.data.value) {
      ...
      await context.overridePermissions("https://mbasic.facebook.com/", [
        "notifications",
      ]);
      const page = await browser.newPage();
      await page.setExtraHTTPHeaders({'Accept-Language': 'en-US'});
      ...
      if(page.$('[name="submit[This was me]"]')) {
        ...
        if ((await page.$('button[name="submit[This was me]"]')) !== null) {
          console.log("this was me after geo");
          await page.click("#checkpointSubmitButton");
          await page.waitForTimeout(4000);
        }
